...
class UserSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // Hash passwords once (idempotency safe since bcrypt changes each run)
        $hashedAdminPassword = Hash::make($plainAdminPassword);
        $hashedEmployeePassword = Hash::make($plainEmployeePassword);

        // Create 3 admin users (idempotent)
        for ($i = 1; $i <= 3; $i++) {
            User::updateOrCreate(
                ['email' => "admin_{$i}@example.com"], // Unique check
                [
                    'name' => "Admin User {$i}",
        ...
        // Create 50 employee users (idempotent)
        for ($i = 1; $i <= 50; $i++) {
            User::updateOrCreate(
                ['email' => "employee_{$i}@example.com"], // Unique check
                [
